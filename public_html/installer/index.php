...
function base_url($slug = '') {
    return str_replace('installer/','', getRoot().$slug);
}
function input($index, $value = '') {
    $value = (isset($_GET[$index]) or in_array($index, $_GET)) ? $_GET[$index] : $value;
    $value = (isset($_POST[$index]) or in_array($index, $_POST)) ? $_POST[$index] : $value;
    return trim($value);
}
...
<html>
<head>
    <title>CryptoEra - Installation</title>
...
<div class="container">
    <?php if ($step == 1):?>
        <h4>CryptoEra Installation</h4>
        <p>This wizard will work you through CryptoEra installation process , to continue press the continue button</p>
        <p>
    ...
    <?php elseif($step == 2):$error = false;?>
        ...
        <h5>Storage / Folder Permissions</h5>

        <ul class="list-group">
            <li class="list-group-item justify-content-between">
                storage/
                <?php if(!is_writable('../storage/')):$error= true;?>
        ...
        <?php if(!$error):?>
            <form action="<?php echo url('?step=3')?>" method="POST">
                <input type="text" name="passed" value="12" style="display: none">
                <input type="hidden" name="permission_passed" value="1"/>
    ...
    <?php elseif($step == 3):?>
        <form action="<?php echo url('?step=3')?>" method="POST">
            ...
            <div class="form-group">
                <label>Exchangerate-api.com APP ID</label>
                <input required class="form-control" name="app_id" value="<?php echo input('app_id')?>" type="text"/>
                <span>Fiat currencies rates are pulled from https://www.exchangerate-api.com. Please sign up and get a free API key (App ID),
                <a href="" onclick="window.open('https://www.exchangerate-api.com')">Signup here</a></span>
            </div>
            ...
            <button  class="btn btn-info" style="margin: 30px 0">Install</button>
        </form>

    <?php elseif($step == 4):?>
        <div class="alert alert-success">Congratulations!!, CryptoEra has been installed successfully on your server</div>

        <h6>Setup <strong>Cron Job</strong></h6>
        <hr/>
        <p>
            CryptoEra needs to periodically update cryptocurrency market data from various internet sources.

            <br/> Cron task should run once per minute. Complete cron task entry should be similar to:
        </p>

        <p>
            <code>* * * * * wget --quiet -O /dev/null http://www.example.com/cron/run</code>
        </p>
        <p>Don't forget to replace http://www.example.com/ with the actual URL of your app.</p>

        <hr/>
        <div class="alert alert-danger">Clicking the button below will take something to update data from api, don't terminate it and make sure your internet connection is working fine</div>
        <p>
            <a href="<?php echo base_url('cron/run?install=true')?>" class="btn btn-primary">Update Market Data & Finish installation</a>
        </p>
    <?php endif?>
</div>
</body>
