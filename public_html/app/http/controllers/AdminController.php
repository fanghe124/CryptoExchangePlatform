...
class AdminController extends Controller {
    ...
    public function settings() {
        ...
        if (request_is_post()) {
            ...
            if ($val) {
                ...
                /**if ($logo = input_file('site_logo_dark')) {
                    $uploader = new Uploader($logo);
                    $uploader->setPath('logo/');
                    if ($uploader->passed()) {
                        $val['site-logo-dark'] = $uploader->uploadFile()->result();
                    } else {
                        $errorMessage = $uploader->getError();
                    }
                }**/
    ...
    public function module() {
        ...
        if ($action = input('action')) {
            if($action == 'enable') {
                enable_module(input('module'));
            } else {
                disable_module(input('module'));
            }
        }
        return $this->render(view('main::admin/module', array('message' => $message)));
    }
    ...
    public function currencies() {
        $this->setTitle(lang('currency-manager'));
        $offset = input('offset', 0);
        $term = input('term');
        if (request_is_post()) {
            if (input('add_currency')) {
                save_currency();
                redirect(url('admincp/currencies'));
            }

            if (input('edit_currency')) {
                save_currency(input('edit_currency'));

            }

        }
        if (input('action') == 'delete') {
            delete_currency(input('id'));

        }
        $currencies = get_currency_list($offset, $term);
        return $this->render(view('main::admin/currency', array('currencies' => $currencies)));
    }
    ...
    public function updateSystem() {
        ...
        foreach($modules as $module) {
            $open = @opendir(path('module/'.$module.'/update/'));
            while($read = readdir($open)) {
                if (substr($read, 0, 1) !='.') {
                    try{
                        db()->query(file_get_contents(path('module/'.$module.'/update/'."$read")));
                    } catch(Exception $e){}
                }
            }
        }
