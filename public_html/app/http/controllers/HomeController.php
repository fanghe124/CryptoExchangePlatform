...
class HomeController extends Controller {
    public function home() {
        ...
        return $this->render(view('main::home/home'));
    }
	 ...
	 public function logreg() {
        ...
        return $this->render(view('main::home/logreg'));
    }
	
    public function index() {
        $this->setTitle(lang('live-cryptocurrecies-title'));
        app()->activeMenu = 'market';
        $limit = config('coin.list.limit', input('limit', 100));
        $order = config('coin.order', input('order', 'market_cap'));
        $orderType = input('order_type', 'DESC');
        $offset = input('offset', 0);
        $type = segment(0, 'coins');
        $coins = get_coins($order,$orderType,$limit,$offset,$type);
    ...
    public function signup() {
        ...
        $userid = register_member($email,$password);
    ...
    public function activate() {
        ...
        if ($query->rowCount() > 0) {
            $user = $query->fetch(PDO::FETCH_ASSOC);
            db()->query("UPDATE members SET active=? WHERE id=? ", 1, $user['id']);
