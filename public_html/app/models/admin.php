...
function get_theme_options() {
    $theme = get_theme();
    $result = array();
    $file = path('public/styles/'.$theme.'/options.php');
    if (file_exists($file)) $result = include_once($file);
    return $result;
}

function get_theme_list() {
    $path = path('public/styles');
    $handle = opendir($path);
    $result = array();
...
function get_page_list(){
    $query = db()->query("SELECT * FROM pages ORDER BY id");
    return $query->fetchAll(PDO::FETCH_ASSOC);
}

function save_page($id = null) {
    $title = input('title');
    ...
    if ($id) {
        db()->query("UPDATE pages SET title=?,description=?,content=?,menu=? WHERE id=? ", $title,$desc,$content,$menu,$id);

    } else {
        $slug = toAscii($title);
        $slug = ($slug) ? $slug : md5(time());
        db()->query("INSERT INTO pages (slug,title,description,content,menu)VALUES(?,?,?,?,?)",$slug,$title,$desc,$content,$menu);
    }
...
function get_menu_list(){
    $query = db()->query("SELECT * FROM menus ORDER BY id ASC");
    return $query->fetchAll(PDO::FETCH_ASSOC);
}

function save_menu($id = null) {
    $title = input('title');
    $icon = input('icon');
    $link = input('link');
   $ajaxify = input('ajaxify', 0);
    $newTab = input('new_tab', 0);
    $visible = input('visible', 0);


    if ($id) {
        db()->query("UPDATE menus SET title=?,icon=?,ajaxify=?,new_tab=?,visible=?,link=? WHERE id=? ", $title,$icon,$ajaxify,$newTab,$visible,$link,$id);

    } else {

        db()->query("INSERT INTO menus (link,title,icon,ajaxify,new_tab,visible)VALUES(?,?,?,?,?,?)",$link,$title,$icon,$ajaxify,$newTab,$visible);
    }
...
function get_main_menu_list(){
    try{
        $query = db()->query("SELECT * FROM menus WHERE visible=?", 1);
        return $query->fetchAll(PDO::FETCH_ASSOC);
    } catch(Exception $e) {return array();}
}

function get_more_menu_list(){
    try {
        $query = db()->query("SELECT * FROM menus WHERE visible=?", 0);
        $menus =  $query->fetchAll(PDO::FETCH_ASSOC);

        $query =  db()->query("SELECT * FROM pages WHERE menu=? ", 1);
        while($fetch  = $query->fetch(PDO::FETCH_ASSOC)) {
            $menus[] = array(
                'title' => $fetch['title'],
                'icon' => '',
                'ajaxify' => 1,
                'link' => page_link($fetch)
            );
        }
        return $menus;
    } catch(Exception $e) {
...
function get_page($id) {
    $query = db()->query("SELECT * FROM pages WHERE id=? OR slug=? ", $id,$id);
    return $query->fetch(PDO::FETCH_ASSOC);
}

function enable_module($module) {
    $modules = explode(',', config('installed-modules',''));
    if (!in_array($module, $modules)) {
        $modules[] = $module;
    }
    $open = @opendir(path('module/'.$module.'/install/'));
    while($read = readdir($open)) {
        if (substr($read, 0, 1) !='.') {
            try{
                db()->query(file_get_contents(path('module/'.$module.'/install/'."$read")));
            } catch(Exception $e){}
        }
    }
    return save_admin_settings(array('installed-modules' => implode(',', $modules)));
}
...
function disable_module($module) {
    ...
    foreach($modules as $m) {
        if ($module != $m) $newModules[] = $m;
    }
    $open = @opendir(path('module/'.$module.'/uninstall/'));
    while($read = readdir($open)) {
        if (substr($read, 0, 1) !='.') {
            try{
                db()->query(file_get_contents(path('module/'.$module.'/uninstall/'."$read")));
            } catch(Exception $e){}
        }
    }
    return save_admin_settings(array('installed-modules' => implode(',', $newModules)));
}

function get_currency_list($offset, $term) {
    $sql = "SELECT * FROM currencies WHERE id!='' ";
    if ($term) {
        $sql .= " AND (name LIKE '%$term%' OR symbol LIKE '%$term%' OR code LIKE '%$term%') ";
    }
...
function save_currency($id = null) {
    $name = input('name');
    $code = input('code');
    $symbol = input('symbol');
    ...
    if ($id) {
        db()->query("UPDATE currencies SET name=?,code=?,symbol=?,country_code=?,rate=?,last_time=? WHERE id=? ",
            $name,$code,$symbol,$country_code,$rate,time(),$id);

    } else {

        db()->query("INSERT INTO currencies (name,code,symbol,country_code,rate,last_time)VALUES(?,?,?,?,?,?)",
            $name,$code,$symbol,$country_code,$rate,time());
    }
