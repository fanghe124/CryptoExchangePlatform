...
class Application {
    ...
    public  $COINS_MARKET_DATA_URL = 'https://api.coinmarketcap.com/v1/ticker/?limit=0';
    public  $CURRENCIES_MARKET_DATA_URL = 'https://v3.exchangerate-api.com/bulk/%s/%s';
    public $COINS_HISTORICAL_MARKET_DATA_URL = 'https://min-api.cryptocompare.com/data/%s?fsym=%s&tsym=%s&limit=%d&aggregate=%d&allData=%s';
    ...
    public function run() {
        ...
        include_once path('app/http/routes.php');
        $this->loadModules();

        //auto detech user country currency
        if (!isset($_COOKIE['base_currency'])) {
            try{
                $api = config('user-country-api', '');
               if ($api) {
                   $ip = get_ip();
                   $usercountry = @file_get_contents("https://usercountry.com/v1.0/json/$ip?token=$api");
                   $usercountry = @json_decode($usercountry);
                   if($usercountry) {
                       $this->countryCurrency = $usercountry->currency->code;
                       setcookie("base_currency", $this->countryCurrency, time() + 30 * 24 * 60 * 60, config('cookie_path'));
                   }
               }
            } catch(Exception $e) {}
        } else {
            $this->countryCurrency = $_COOKIE['base_currency'];
        }
        Router::run();
    }

    private function loadModels() {
        include path('app/models/admin.php');
        include path('app/models/user.php');
        include path('app/models/crypto.php');
        include path('app/models/portfolio.php');
    ...
    public function loadModules() {
        ...
        foreach($modules as $module) {
           if ($module) {
               $modulePath  = $basePath .$module.'/init.php';
               include_once $modulePath;
               Translation::getInstance()->addNamespace($module, $basePath .$module."/languages/");
               View::addNamespace($module,$basePath .$module."/views/");
           }
        }
        return true;
    }
    ...
    public function listModules() {
        ...
        while($read = @readdir($handle)) {
            if (substr($read, 0,1) != '.') {
                $info = include $basePath .$read.'/info.php';
                $modules[$read] = $info;
            }
        }
        ...
        if ($filepath) {
            ...
            if ($dir) {
                try{
                    ...
                    @mkdir(path('storage/cache/'.$key.'/'), 0777, true);
                }catch(Exception $e) {}
            }
...
class Translation {
    ...
    public function init() {
        $this->addNamespace("global", path("app/languages/"));
...
class Router {
    ...
    public static function load($route, $active = true, $filter = false, $permission = false) {
        ...
        $modulesPath = path("app/http/controllers/");
...
class Layout {
    ...
    public function __construct() {
        ...
        $this->favicon = url(config('site-favicon', 'public/assets/images/favicon.png'));
    }
    ...
    public function setTitle($title = "") {
        $titleStr = config('site-title', 'CoinRank');
...
class View {
    ...
    public static  function init() {
        static::$defaultTheme = config('theme', 'default');

        static::addNamespace("main", path("app/views/"));
        static::addNamespace("style", path("public/styles/".static::$defaultTheme.'/views/'));
    }
